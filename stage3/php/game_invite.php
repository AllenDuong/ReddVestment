...
// UPDATES GAME INVITE ACCEPTED STATUS (WHETHER THEY HAVE TIMED OUT)
    ...
    if (isset($_POST['accepted'])){
        ...
        $sql = "UPDATE Games SET accepted=0 WHERE time_created>($time-15) AND accepted=1";
    }
    $result = mysqli_query($connection, $sql) or die ('Query Failed: ' . mysql_error());
    if ($result) {
        exit('success');
...
// SHOWS WHO SENT A GAME INVITE TO BE DISPLAYED
    ...
    $user = $_SESSION['username'];
    $time = time();
    $sql = "SELECT player1, subreddit,time_created FROM Games WHERE player2='$user' AND accepted=1 AND time_created>($time-15)";
    $result = mysqli_query($connection, $sql) or die ('Query Failed: ' . mysql_error());
    ...
    if ($tuple){
        ...
        $sql = "SELECT player2, subreddit, time_created FROM Games WHERE player1='$user' AND accepted=1 AND time_created>($time-15)";
        $result = mysqli_query($connection, $sql) or die ('Query Failed: ' .mysql_error());
        $tuple = mysqli_fetch_array($result, MYSQL_ASSOC);
        if ($tuple){
    ...
    if ($tuple){
        ...
        //$sql = "UPDATE Games SET accepted=3 WHERE player1='". $tuple["player1"] ."' AND player2='" . $tuple["player2"] . "' AND subreddit='" . $tuple["subreddit"] . "' AND time_created=" . $tuple["time_created"];
        //$result = mysqli_query($connection, $sql) or die ('Query Failed: ' . mysql_error());
        exit("accepted");
    } else {
        exit("failure");
